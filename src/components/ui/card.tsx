...
export const Card = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "rounded-lg border border-border bg-surface shadow-sm transition-[color,background-color,border-color,box-shadow] duration-200",
        className,
      )}
      {...props}
    />
...
export const CardTitle = forwardRef<HTMLHeadingElement, HTMLAttributes<HTMLHeadingElement>>(
  ({ className, ...props }, ref) => (
    <h3
      ref={ref}
      className={cn("font-display text-body-lg font-semibold text-foreground", className)}
      {...props}
    />
  ),
);
